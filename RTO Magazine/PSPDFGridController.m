...
- (void)settingsButtonPressed {
    if ([self.popoverController.contentViewController isKindOfClass:[SettingsViewController class]]) {
        [self.popoverController dismissPopoverAnimated:YES];
        self.popoverController = nil;
    }else {
        UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"MainStoryboard" bundle:nil];
        
        SettingsViewController *SettingsController = [storyboard instantiateViewControllerWithIdentifier:@"SettingsView"];
        if (PSIsIpad()) {
            self.popoverController = [[UIPopoverController alloc] initWithContentViewController:SettingsController];
            UIBarButtonItem *barSetting = [self.navigationItem.leftBarButtonItems objectAtIndex:1];
            [self.popoverController presentPopoverFromBarButtonItem:barSetting permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
        }else {
...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration {
    ...
    if(toInterfaceOrientation == UIInterfaceOrientationLandscapeLeft || toInterfaceOrientation == UIInterfaceOrientationLandscapeRight){
        NSLog(@"%@",[UIImage imageNamed:@"logo_landscape.png"]);
        [logoImg setImage:[UIImage imageNamed:@"logo_landscape.png"]];
    }else if(toInterfaceOrientation == UIInterfaceOrientationPortrait || toInterfaceOrientation == UIInterfaceOrientationPortraitUpsideDown){
        [logoImg setImage:[UIImage imageNamed:@"logo.png"]];
    }
}
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    UIBarButtonItem *settingButton = [[UIBarButtonItem alloc] initWithTitle:PSPDFLocalize(@"Settings")
                                                                     style:UIBarButtonItemStyleBordered
                                                                    target:self
    ...
    NSArray *buttonsArray = [NSArray arrayWithObjects:optionButton,settingButton, nil];
    [self.navigationItem setLeftBarButtonItems:buttonsArray];
    [[UIDevice currentDevice] beginGeneratingDeviceOrientationNotifications];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(willAnimateRotationToInterfaceOrientation:duration:) name:UIDeviceOrientationDidChangeNotification object:nil];
    // add global shadow

    CGFloat toolbarHeight = self.navigationController.navigationBar.frame.size.height;
    self.shadowView = [[PSPDFShadowView alloc] initWithFrame:CGRectMake(0, -toolbarHeight+92, self.view.bounds.size.width, toolbarHeight)];
    ...
    //shadowView_.backgroundColor = [UIColor redColor];
    ...
    UIView *backgroundTextureView = [[UIView alloc] initWithFrame:CGRectMake(0, 92, self.view.bounds.size.width, self.view.bounds.size.height + toolbarHeight)];
    ...
    backgroundTextureView.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"imgres.jpg"]];

    [self.view insertSubview:backgroundTextureView belowSubview:shadowView_];
    
    // init grid
    if(UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation)){
        logoImg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"logo_landscape.png"]];
    }else{
        logoImg = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"logo_landscape.png"]];
    }
        ...
        if (magazine.isDownloading) {
            ...
            if (magazine.isLocked) {
                PSPDF_IF_IOS5_OR_GREATER(
                                         ...
                                         PSPDFAlertView *weakAlertView = alertView;
